...
struct Exp : public EuclideanExpression {
  RotationQuaternion qC12;
  EuclideanPoint p2;
  Exp(const ProjectionProblem::ConstInput& constInput) : EuclideanExpression(nullptr), qC12(sm::kinematics::quatIdentity()), p2(Eigen::Vector3d::Ones()) {
    (EuclideanExpression&)(*this) = (RotationExpression(&qC12).inverse()) * EuclideanExpression(&p2);
  }
  ...
  void setVars(const typename AslamExpressionSolver<UseNewGenericMatrixStuff>::Problem::Input & input){
    ...
    p2.setParameters(input.p2 * 0.5); // TODO this is a little bit of cheating, but it is not implemented so far to scale a Euclidean expression.
    qC12.setActive(true);
...
struct Exp<true> : public GenericMatrixExpression<3, 1> {
  ...
  DesignVariableGenericVector<3> p2;
  Exp(const ProjectionProblem::ConstInput& constInput)
  {
    ...
    (GenericMatrixExpression<3, 1>&)(*this) = UQ(&qC12).rotate3Vector(GenericMatrixExpression<3,1>(&p2)) * 0.5;
  }
  void setVars(const typename AslamExpressionSolver<true>::Problem::Input & input){
    qC12.setParameters(input.qC12);
    p2.setParameters(input.p2);
    qC12.setActive(true);
    qC12.setBlockIndex(0);
    p2.setActive(true);
    p2.setBlockIndex(2);
    ...
    virtual void solveInto(const AslamExpressionSolver::Problem::Input & input, AslamExpressionSolver::Problem::Output & output, const EvalVariants v) override {
      ...
      switch(v){
        ...
        case EvalVariants::EvalJacobian:
        {
          ...
          output.jP2 = J.block(0, 1)->template block<2, 3>(0, 0);
